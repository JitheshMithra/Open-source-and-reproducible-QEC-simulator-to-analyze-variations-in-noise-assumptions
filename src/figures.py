...
print("Running fig 1...")
resultsBf = distanceSweep(distances=distances, pValues=pValues, trials=trials, seed=seed, noiseType="bitflip", sweepParam="p")
ciBf = bootstrapThreshold(resultsBf, nBootstrap=1000, confidence=0.95)
fig, ax = plt.subplots(figsize=(6, 4))
for d in distances:
    x = [r["physical_error_rate"] for r in resultsBf[d]]
    y = [r["LER"] for r in resultsBf[d]]
    err = [r["stderr"] for r in resultsBf[d]]
    analytical = [analyticalLogicalError(d, p) for p in x]
...
for ax, c in zip(axes, cList):
    ...
    annY = {0.1: 0.43, 0.3: 0.38, 0.5: 0.35}
    textY = {0.1: 0.32, 0.3: 0.30, 0.5: 0.28}

    ax.axvline(crossings[c], color="black", linestyle=":", linewidth=1.2, alpha=0.7)
    ax.annotate(
        f"$p^* = {crossings[c]}$",
        xy=(crossings[c], annY[c]),
        xytext=(crossings[c] + 0.04, textY[c]),
        fontsize=9,
        color="black",
...
ax.errorbar(cVals, t35, yerr=err35, fmt="o-", color="#1f77b4",
           capsize=4, linewidth=2.0, markersize=7, label=r"$d=3$ vs $d=5$")
ax.errorbar(cVals, t57, yerr=err57, fmt="s--", color="#ff7f0e",
...
cMids = [0.15, 0.25, 0.35, 0.45]
sVals = [0.389, 0.557, 0.491, 0.800]
sErrs = [0.071, 0.078, 0.079, 0.104]
labels = ["0.1→0.2", "0.2→0.3", "0.3→0.4", "0.4→0.5"]
# endpoints solid, middle faded
alphas = [1.0, 0.35, 0.35, 1.0]
pointColors = ["#d62728", "#aaaaaa", "#aaaaaa", "#d62728"]
fig, ax = plt.subplots(figsize=(6, 4))
for i in range(4):
    ax.errorbar(cMids[i], sVals[i], yerr=sErrs[i],
               fmt="o", color=pointColors[i], capsize=5,
               capthick=1.5, linewidth=1.5, markersize=8,
               alpha=alphas[i])
# shade endpoint ranges
ax.axhspan(sVals[0] - sErrs[0], sVals[0] + sErrs[0],
          alpha=0.08, color="#1f77b4", label="$c=0.1\\to0.2$ range")
ax.axhspan(sVals[3] - sErrs[3], sVals[3] + sErrs[3],
...
ax.errorbar(dVals, [bf[d][0] for d in dVals],
           yerr=[bf[d][1] for d in dVals],
           marker="o", color="#1f77b4", capsize=4,
           linewidth=2.0, markersize=7, label="Bitflip")
ax.errorbar(dVals, [co[d][0] for d in dVals],
           yerr=[co[d][1] for d in dVals],
